...
npm view harness-kit             # 名字可用性 / 已存在则看是否本账号可接管
npm pack --dry-run               # 看将发布哪些文件、体积

# 2. 版本 + 标签
npm version patch -m "chore: release v%s"   # 或 minor；commit + tag 由 npm version 完成

# 3. 发布（provenance：sigstore 签名，04 A4 供应链信任锚）
npm publish --provenance --access public

# 4. 验证
npm view harness-kit@latest dist-tags version
npm i -g harness-kit && harness-kit agent-prompt --role presale   # 装后用 agent-prompt 自检
```
